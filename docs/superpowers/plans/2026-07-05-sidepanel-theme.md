...
- Serif (`Source Serif 4`) is used **only** for reading content (the AI-generated overview paragraph) **and the panel's own `.app-title` branding header** (a deliberate wordmark-style exception, not a reading-content or functional-label use — matches the original mockup's bookish display treatment). Sans (`Instrument Sans`) is used for all other interface chrome (buttons, section labels, meta text). Mono (`IBM Plex Mono`) is used **only** for small-caps eyebrow/metadata labels. Never mix these roles outside the one named exception.
